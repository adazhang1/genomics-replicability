...
`prespecified.yml` contains the environment used to run model inference.  Note that this environment uses package versions that are more recent than what is specified in the requirements file.  This environment was built package-by-package to try to resolve the conflicts we were experiencing with the `requirements.txt` file.  While we were ultimately able to generate a set of versions without conflicts similar to what was originally specified by Enformer (which we provide in `requirements.txt`), model inference results were generated with the environment defined by `prespecified.yml`.  You can use `conda env create -f prespecified.yml` to build a conda environment from this file.
